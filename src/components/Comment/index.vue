  ...
  <div id="comment" ref="comment">
    ...
    <comment-list v-if="cacheData.length > 0" ref="comment-list">
      ...
      <comment-item
        ...
        :data ="`comment-${comment.commentsId}`"
        :user="user"
        :comment="comment"
          ...
          <comment-list sub>
            ...
            >
              ...
              <comment-form
                ...
                :comment="child"
                :parent="comment"
                :placeholder="`@${child.user && child.user.username}...`"
                :upload-img="uploadImg"
                @form-delete="deleteForm"
                @form-submit="formSubmit"
              />
...
import { MixinToken } from './components/tokenMixin'
export default {
  name: 'Comment',
  ...
  methods: {
    ...
    validate({ key, value }) {
      const map = {
        ...
        createTime: {
          ...
          message: 'createTime is not a valid date.',
        },
      }
    ...
    // * 删除评论或回复
    handleCommentDelete({ id, comment, parent = null }) {
    ...
    addComment(id, rawData) {
      ...
      if (commentIndex === 'root') {
        this.cacheData = [ ...this.cacheData, rawData] 
        // this.cacheData.push(rawData)
      } else {
        const comment = this.cacheData[commentIndex]
         comment.children= [ ...comment.children,rawData ] 
    ...
    scrollIntoViewDom(ref) {
         this.$nextTick(()=>{
            let parendArr = this.$refs['comment-children'];
            let flagId = 'commentId-'+ref;
            parendArr.filter((item,i)=>{
              if ( item.$refs[flagId]) {
                item.$refs[flagId].scrollIntoView(
                  {
                    behavior: 'smooth',
                    block: 'center'
                  }
